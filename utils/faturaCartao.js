...
function dataFechamentoDoCiclo(referencia, diaFechamento, diaVencimento) {
  const anoVenc = parseInt(referencia.slice(0, 4), 10);
  const mesVenc = parseInt(referencia.slice(5, 7), 10) - 1;
  let anoFech = anoVenc;
  let mesFech = mesVenc;
  if (diaVencimento < diaFechamento) {
    mesFech -= 1;
    if (mesFech < 0) { mesFech = 11; anoFech -= 1; }
  }
  const ultimoDia = new Date(anoFech, mesFech + 1, 0).getDate();
  const diaFinal = Math.min(diaFechamento, ultimoDia);
  return `${anoFech}-${String(mesFech + 1).padStart(2, '0')}-${String(diaFinal).padStart(2, '0')}`;
}

module.exports = { computarCicloFatura, proximoCicloFatura, dataFechamentoDoCiclo };
