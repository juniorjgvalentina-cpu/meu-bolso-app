...
async function initDb() {
  ...
  try {
    ...
    // Cartoes de credito da conta. Cada cartao tem seu proprio dia de
    // vencimento de fatura - usado pra gerar a pendencia recorrente "Pagar
    // fatura do [nome]" automaticamente.
    await client.query(`
    ...
    // Qual cartao foi usado nessa compra no credito - pra quem tem mais de
    // um cartao saber de qual fatura cada gasto faz parte.
    await client.query(`
      ALTER TABLE lancamentos ADD COLUMN IF NOT EXISTS cartao_id INTEGER REFERENCES cartoes(id) ON DELETE SET NULL;
    `);

    // Quando a fatura desse lancamento foi paga de verdade. Diferente de
    // data_lancamento (que fica sempre no mes da COMPRA), esse campo marca
    // o mes em que o dinheiro realmente saiu do bolso - essencial pra nao
    // contar o mesmo gasto duas vezes (nem deixar de contar nenhuma vez)
    // quando a fatura e paga num mes diferente do mes da compra.
    await client.query(`
    ...
    // Se essa pendencia for a fatura de um cartao (em vez de uma conta comum
    // tipo aluguel), aponta pra qual cartao - resolver ela marca os
    // lancamentos daquele cartao como pagos, em vez de criar um lancamento novo.
    await client.query(`
