...
async function autenticar(req, res, next) {
  ...
  try {
    const membroResult = await pool.query(
      `SELECT id, nome, papel, conta_id FROM membros WHERE token_acesso = $1`,
      [token]
    );
    ...
    // Bloqueia lancamentos novos se a assinatura estiver vencida,
    // mas nao impede o resto do sistema de funcionar (dados continuam salvos).
    if (conta.status_assinatura === 'vencido' && req.method !== 'GET') {
