...
router.post('/lancamentos', autenticar, async (req, res) => {
  ...
  try {
    ...
    // Divide o valor total pelas parcelas (em centavos, pra nao perder
    // precisao) - a ultima parcela absorve o resto da divisao, pra soma
    // das parcelas bater exatamente com o valor total digitado.
...
// Devolve os lancamentos no credito desse mes que ainda nao tiveram a
// fatura paga - usado pra mostrar a aba "Fatura do Cartao" na Planilha.
router.get('/lancamentos/fatura', autenticar, async (req, res) => {
  try {
    let query = `
      SELECT * FROM lancamentos
      WHERE conta_id = $1 AND forma_pagamento = 'credito' AND fatura_paga = FALSE
...
// Marca a fatura do mes atual como paga - so a partir daqui o valor sai
// de verdade do saldo em dinheiro (antes disso, o gasto ja conta na
// categoria, mas nao no saldo, porque o dinheiro ainda nao saiu do bolso).
router.put('/lancamentos/fatura/pagar', autenticar, async (req, res) => {
  try {
    let query = `
    ...
    query += ` RETURNING id`;
    const result = await pool.query(query, params);
    res.json({ status: 'ok', quantidade: result.rows.length });
  } catch (err) {
    console.error('Erro ao pagar fatura:', err);
    res.status(500).json({ erro: 'Erro ao marcar fatura como paga.' });
  }
});
