...
router.post('/lancamentos', autenticar, async (req, res) => {
  const {
    descricao, valor, tipo_movimento,
  ...
  if (tipo_movimento === 'saida') {
    if (!CATEGORIAS_VALIDAS.includes(categoria)) {
      return res.status(400).json({ erro: 'Categoria invalida. Use fixo, superfluo ou imprevisto.' });
    }
  ...
  try {
    ...
    // Aprende no dicionario - so fixo/superfluo, nunca imprevisto
    if (tipo_movimento === 'saida' && (categoriaFinal === 'fixo' || categoriaFinal === 'superfluo')) {
    ...
    for (let i = 0; i < totalParcelas; i++) {
      ...
      const result = await client.query(
        `INSERT INTO lancamentos
         ...
         RETURNING *`,
        [
          req.conta.id, req.membro.id, descricao.trim(), valorNum, tipo_movimento,
...
// e na tela de Planilha). Dono e membros com visao completa veem tudo da
// conta; membros restritos veem so os proprios lancamentos.
