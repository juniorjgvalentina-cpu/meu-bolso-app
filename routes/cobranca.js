...
router.post('/cobranca/gerar', autenticar, apenasDono, async (req, res) => {
  ...
  try {
    ...
    if (pendenteResult.rows.length > 0) {
      ...
      if (c.mp_payment_id) {
        const checagemResponse = await fetch(`${MP_API}/${c.mp_payment_id}`, {
          headers: { 'Authorization': `Bearer ${process.env.MP_ACCESS_TOKEN}` }
        });
        const checagemData = await checagemResponse.json();
        if (checagemResponse.ok && checagemData.status === 'approved') {
