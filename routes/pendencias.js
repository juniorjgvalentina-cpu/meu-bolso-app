...
const { proximoCicloFatura } = require('../utils/faturaCartao');
...
router.post('/pendencias', autenticar, async (req, res) => {
  const { tipo, nome, data_vencimento, valor, recorrente, total_ocorrencias } = req.body;
  ...
  let totalOcorrenciasNum = null;
  if (recorrente && total_ocorrencias !== undefined && total_ocorrencias !== null && total_ocorrencias !== '') {
    totalOcorrenciasNum = parseInt(total_ocorrencias, 10);
    if (isNaN(totalOcorrenciasNum) || totalOcorrenciasNum < 1 || totalOcorrenciasNum > 120) {
      return res.status(400).json({ erro: 'Numero de vezes invalido.' });
    }
  }
  ...
  try {
    const result = await pool.query(
      ...
      [req.conta.id, req.membro.id, tipo, nome.trim(), valorNum, data_vencimento, !!recorrente, totalOcorrenciasNum]
    );
  ...
  try {
    ...
    if (pendencia.cartao_id) {
      const cartaoResult = await client.query(
        `SELECT dia_vencimento FROM cartoes WHERE id = $1`,
        [pendencia.cartao_id]
      );
      const diaVencimentoCartao = cartaoResult.rows[0] ? cartaoResult.rows[0].dia_vencimento : 1;

      let marcados;
      if (pendencia.fatura_referencia) {
        marcados = await client.query(
          `UPDATE lancamentos SET fatura_paga = TRUE, fatura_paga_em = NOW()
           WHERE conta_id = $1 AND cartao_id = $2 AND fatura_paga = FALSE AND fatura_referencia = $3
           RETURNING id, valor`,
          [req.conta.id, pendencia.cartao_id, pendencia.fatura_referencia]
        );
      } else {
        marcados = await client.query(
          `UPDATE lancamentos SET fatura_paga = TRUE, fatura_paga_em = NOW()
      ...
      if (pendencia.recorrente) {
        ...
        if (jaExisteProxima.rows.length === 0) {
          let proximaData, proximaReferencia;
          if (pendencia.fatura_referencia) {
            const proximo = proximoCicloFatura(pendencia.fatura_referencia, diaVencimentoCartao);
            proximaData = proximo.dataVencimento;
            proximaReferencia = proximo.referencia;
          } else {
            proximaData = somarMeses(new Date(pendencia.data_vencimento), 1).toISOString().slice(0, 10);
          ...
          await client.query(
            ...
            [req.conta.id, req.membro.id, pendencia.tipo, pendencia.nome, proximaData, pendencia.cartao_id, proximaReferencia]
          );
        }
      }
    ...
    const atingiuLimite = pendencia.total_ocorrencias && pendencia.ocorrencia_atual >= pendencia.total_ocorrencias;

    if (pendencia.recorrente && !atingiuLimite) {
      const jaExisteProxima = await client.query(
        `SELECT 1 FROM contas_pendentes
      ...
      if (jaExisteProxima.rows.length === 0) {
        ...
        await client.query(
          ...
          [
            ...
            pendencia.total_ocorrencias, (pendencia.ocorrencia_atual || 1) + 1
          ]
        );
      }
    }
...
router.put('/pendencias/:id', autenticar, async (req, res) => {
  const { nome, data_vencimento, valor, dia_fechamento } = req.body;
  ...
  try {
    ...
    let nomeFinalPendencia = nome.trim();

    if (pendencia.cartao_id) {
      const novoDia = new Date(data_vencimento + 'T00:00:00').getDate();

      if (dia_fechamento !== undefined && dia_fechamento !== null && dia_fechamento !== '') {
        const novoFechamento = parseInt(dia_fechamento, 10);
        if (!novoFechamento || novoFechamento < 1 || novoFechamento > 31) {
          return res.status(400).json({ erro: 'Dia de fechamento invalido.' });
        }
        await pool.query(
          `UPDATE cartoes SET nome = $1, dia_vencimento = $2, dia_fechamento = $3 WHERE id = $4`,
          [nome.trim(), novoDia, novoFechamento, pendencia.cartao_id]
        );
      } else {
        await pool.query(
          `UPDATE cartoes SET nome = $1, dia_vencimento = $2 WHERE id = $3`,
          [nome.trim(), novoDia, pendencia.cartao_id]
        );
      }
      nomeFinalPendencia = `Fatura do cartão ${nome.trim()}`;
    }

    const result = await pool.query(
      `UPDATE contas_pendentes SET nome = $1, data_vencimento = $2, valor = $3 WHERE id = $4 RETURNING *`,
      [nomeFinalPendencia, data_vencimento, pendencia.cartao_id ? null : valorNum, req.params.id]
    );
