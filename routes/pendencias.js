...
router.post('/pendencias/:id/resolver', autenticar, async (req, res) => {
  ...
  try {
    ...
    // Caso especial: essa pendencia e a fatura de um cartao (nao uma conta
    // comum tipo aluguel). Nao cria lancamento novo - o gasto ja foi
    // categorizado na hora de cada compra. So marca os lancamentos daquele
    // cartao como pagos, o que ja desconta do saldo em dinheiro sozinho.
    ...
    if (tipoMovimento === 'saida') {
      ...
      // Pendencia recorrente ("todo mes") ja e considerada Fixo automaticamente, sem perguntar.
    ...
    // Mesma divisao por centavos usada em routes/lancamentos.js - a ultima
    // parcela absorve o resto da divisao pra soma bater com o valor total.
    ...
    // Se for recorrente, cria a proxima pendencia do mes seguinte - mas so se
    // ela ainda nao existir (o job diario pode ja ter gerado, se a pessoa
    // atrasou o pagamento por mais de um mes).
    if (pendencia.recorrente) {
      const jaExisteProxima = await client.query(
        `SELECT 1 FROM contas_pendentes
...
// Edita uma pendencia (nome, data de vencimento, valor). Se for a fatura de
// um cartao, tambem atualiza o dia de vencimento salvo no cartao - assim as
// proximas faturas geradas automaticamente ja nascem na data nova.
