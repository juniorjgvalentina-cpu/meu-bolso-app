...
router.post('/login', async (req, res) => {
  ...
  try {
    const membroResult = await client.query(
      `SELECT m.id, m.nome, m.telefone, m.papel, m.conta_id
...
router.post('/recuperar', async (req, res) => {
  ...
  try {
    const membroResult = await client.query(
      `SELECT id, nome, telefone, papel, conta_id FROM membros WHERE telefone = $1`,
      [telefoneLimpo]
    );
    ...
    res.json({
      token: novoToken,
      membro: { id: membro.id, nome: membro.nome, telefone: membro.telefone, papel: membro.papel },
      conta
    });
  } catch (err) {
...
// Edita nome e/ou telefone do proprio perfil (tela de Configuracoes).
router.put('/perfil', autenticar, async (req, res) => {
  const { nome, telefone } = req.body;

  if ((!nome || !nome.trim()) && (!telefone || !telefone.trim())) {
    return res.status(400).json({ erro: 'Informe nome ou telefone pra atualizar.' });
  }
  ...
  valores.push(req.membro.id);

  try {
    const result = await pool.query(
      `UPDATE membros SET ${campos.join(', ')} WHERE id = $${indice} RETURNING id, nome, telefone, papel`,
      valores
    );
    res.json({ membro: result.rows[0] });
  } catch (err) {
    if (err.code === '23505') {
      return res.status(400).json({ erro: 'Esse telefone ja esta em uso nessa conta.' });
    }
...
// Exclui o cadastro permanentemente. Se for o dono da conta, apaga a conta
// inteira (todos os membros, lancamentos, tudo - por causa do ON DELETE
// CASCADE do banco). Se for um membro comum, apaga so o proprio login e os
// proprios lancamentos, sem mexer na conta compartilhada dos outros.
router.delete('/perfil', autenticar, async (req, res) => {
  try {
    if (req.membro.papel === 'dono') {
      ...
      await pool.query(`DELETE FROM membros WHERE id = $1`, [req.membro.id]);
    }
    res.json({ status: 'ok' });
  } catch (err) {
    console.error('Erro ao excluir cadastro:', err);
    res.status(500).json({ erro: 'Erro ao excluir cadastro. Tente novamente.' });
  }
});
