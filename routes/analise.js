...
async function totaisDoMes(contaId, membroId, apenasProprio, offsetMeses) {
  let query = `
    SELECT tipo_movimento, categoria, forma_pagamento, fatura_paga, SUM(valor) AS total
    FROM lancamentos
    WHERE conta_id = $1
  ...
  query += ` GROUP BY tipo_movimento, categoria, forma_pagamento, fatura_paga`;

  const result = await pool.query(query, params);

  const totais = { entrada: 0, fixo: 0, superfluo: 0, diaadia: 0, imprevisto: 0 };
  let totalPagoDoMes = 0;
  ...
  let crossQuery = `
    ...
    WHERE conta_id = $1 AND forma_pagamento = 'credito' AND fatura_paga = TRUE
      AND date_trunc('month', fatura_paga_em) = date_trunc('month', CURRENT_DATE + ($2 || ' months')::interval)
      AND date_trunc('month', data_lancamento) != date_trunc('month', CURRENT_DATE + ($2 || ' months')::interval)
  `;
  const crossParams = [contaId, offsetMeses];
  if (apenasProprio) {
    crossQuery += ` AND membro_id = $3`;
    crossParams.push(membroId);
  }
  const crossResult = await pool.query(crossQuery, crossParams);
  const totalCrossMes = Number(crossResult.rows[0].total);

  totais.totalPago = totalPagoDoMes + totalCrossMes;
  totais.totalCrossMes = totalCrossMes;
  totais.saldo = totais.entrada - totais.totalPago;
  return totais;
}
