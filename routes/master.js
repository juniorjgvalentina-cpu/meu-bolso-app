...
// Comparacao resistente a timing attack - evita que alguem descubra a senha
// aos poucos medindo quanto tempo cada tentativa demora pra responder.
function comparacaoSegura(a, b) {
  const bufA = Buffer.from(String(a || ''));
  const bufB = Buffer.from(String(b || ''));
  if (bufA.length !== bufB.length) {
    // ainda compara algo de tamanho igual, so pra nao vazar o tamanho certo por tempo
...
function credenciaisValidas(usuario, senha) {
  if (!process.env.ADMIN_USER || !process.env.ADMIN_PASS) return null; // nao configurado
  return comparacaoSegura(usuario, process.env.ADMIN_USER) && comparacaoSegura(senha, process.env.ADMIN_PASS);
}
...
function autenticarMaster(req, res, next) {
  ...
  const valido = credenciaisValidas(usuario, senha);
  if (valido === null) {
...
router.put('/master/contas/:id/marcar-pago', autenticarMaster, async (req, res) => {
  ...
  if (!contaId || contaId <= 0) {
    return res.status(400).json({ erro: 'Conta invalida.' });
  }

  try {
    const contaResult = await pool.query(`SELECT * FROM contas WHERE id = $1`, [contaId]);
...
// Exclui uma conta inteira do banco (dono + membros + lançamentos + tudo,
// via cascade) - usado pra limpar contas de teste/pré-cadastro que ninguém
// usa. Requer confirmação dupla no front antes de chamar aqui.
router.delete('/master/contas/:id', autenticarMaster, async (req, res) => {
  const contaId = parseInt(req.params.id, 10);
  if (!contaId || contaId <= 0) {
    return res.status(400).json({ erro: 'Conta invalida.' });
  }
  ...
  try {
    const result = await pool.query(`DELETE FROM contas WHERE id = $1 RETURNING id`, [contaId]);
    if (result.rows.length === 0) {
      return res.status(404).json({ erro: 'Conta nao encontrada.' });
    }
    res.json({ status: 'ok', excluida: result.rows[0].id });
  } catch (err) {
    console.error('Erro ao excluir conta master:', err);
